...
因为 ArXiv 按提交时间降序排序（新 → 旧）：
- 使用滑动窗口检测最近 20 篇论文状态
- **只有抓到至少一篇新论文后**，才启用提前停止判断
- 只有当**最近 20 篇全部都已经在数据库中**时，才会真正提前停止
- 如果本次同步从头到尾一篇新论文都没抓到，会遍历完所有 `max_results` 才结束，保证不会漏抓

这种设计保证了：
- 正常增量同步：依然能快速提前停止，节省大量时间
- 意外中断重启：即使前 N 篇全已存在，也会继续走到最后，保证不会漏掉后面未抓取的论文
- **已经发现的新论文一定会全部抓取完成，不会漏抓**
